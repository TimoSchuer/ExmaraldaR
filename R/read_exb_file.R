...
read_exb_file <- function(path, readAnn=TRUE,addDescription= FALSE, addMetaData= FALSE, addIPNumber=TRUE,IPEndSign= c("|",".",";",",",",","?","=","-")){
  ...
  if(addDescription==TRUE & length(xml2::xml_find_all(file,".//tier[@type='d']"))!=0){
      ...
      ##join annnotations that are alligned by Start,End and Speaker
      #check if annotations are per spekaer or not speaker assigned
      if(length(dplyr::intersect(unique(events$Speaker), unique(descriptions$Speaker)))==0){##keine Sprecher gleich
        exb <- dplyr::left_join(events, descriptions, by=c("Start", "End"), suffix= c("",".y") ) %>% dplyr::select(-Speaker.y)
      }else if(dplyr::setequal(unique(events$Speaker), unique(descriptions$Speaker))|length(dplyr::setdiff(unique(descriptions$Speaker), unique(events$Speaker)))==0){ ##alle Sprecher gleich oder alle Sprecher der Annotationsspuren in Transkripionsspuren
        exb <- dplyr::left_join(events, descriptions, by=c("Start", "End","Speaker"), suffix= c("",".y"))
      }else{##teils teils
        ##Speaker that have a annotation tier but not a transcription tier
        diff <- dplyr::setdiff(unique(descriptions$Speaker), unique(events$Speaker))
        exb <- descriptions %>%  dplyr::filter(Speaker %in% diff) %>% dplyr::select(-Speaker) %>% dplyr::left_join(events, ., by=c("Start", "End"), suffix= c("",".y") ) #assign them by time
        if ("Speaker.y" %in% names(exb)){ # some cosmetics
          exb <- exb%>% dplyr::select(-Speaker.y)
        }
        exb2 <- descriptions %>%  dplyr::filter(!Speaker %in% diff)  %>% dplyr::left_join(events, ., by=c("Start", "End","Speaker"), suffix= c("",".y") ) # assinge them by time and speaker
        exb <- dplyr::bind_rows(exb,exb2) %>% dplyr::arrange(EventID)
      }
      ##check for annotations that are left out
      if(nrow(dplyr::anti_join(descriptions,events, by=c("Start", "End")))!=0){
        ##Möglichkeiten: ID vergeben und dann mergen oder nested tibble
        ##ID VERGEBEN:
        ##ToDO: ID vor Annotation setzen statt ID Spalte
        ##Idee: mutate across !Start,End;Speaker & !is.na; str_glue(ID,inhalt)
        ## ABER SUCHE DANN IRGENDWIE kompliziert...besser ID-Spalte pro Annotation?
        multiDes <-  dplyr::anti_join(descriptions,events, by=c("Start", "End")) #%>% mutate(AnnID= seq(1:nrow(.)))
        #  exb <-exb %>%  mutate(AnnID=NA)
        ...
        for (n in 1:length(desCols)) {
          ...
          multiDes_help2 <- multiDes  %>%  dplyr::filter(.,!is.na(.data[[desCols[n]]])) %>% dplyr::mutate("{varname}":=seq(1:nrow(.)), .after= {devCols[n]})
