...
# Loop merges Ips together when there were mutiple rows -------------------
  for (i in nrow(exb):2) {
    if(exb[i,Unit_column]==exb[i-1,Unit_column]){
      exb[i-1,"Text"] <- stringr::str_c(exb[i-1,"Text"],exb[i,"Text"]) # merge text
      exb <- exb[-i,]
...
# get percentage if theres more than  one observation per variable --------
  ...
  colnames <- unique(colnames) #getting Vector of Variable names
