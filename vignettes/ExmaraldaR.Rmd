...
EXMARaLDA is a software package developed and maintained by Thomas Schmidt. Besides the transcription and annotation tool Partitur Editor, it includes a tool for managing corpora(Corpus-Manager) and a tool for searching and analysing (EXAKT). This package only contains a interface between R and the .exb format of the Partitur Editor. Maybe there will be options for segmetented files (.exs). Partitur Editor saves transcritions as an xml-based data format, which makes it possible to extract all relevant informations into a data table. As this is can not be an extensive introduction in the EXMARaLDA-Software package i will in the forthgoing assume that the users of this package know:
